...
export default async function ReviewDetailPage({
  ...
  const [product, stylePresetData] = await Promise.all([
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground">
              Created {product.createdDateLabel}
            </span>
            <span className="text-sm text-muted-foreground">
              {product.aiGenerations.length} AI runs logged
            </span>
          </div>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-2 p-5 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              <Badge
                variant={
                  product.latestPublishResult.success ? "default" : "secondary"
                }
              >
                {product.latestPublishResult.success
                  ? "Last publish succeeded"
                  : "Last publish failed"}
              </Badge>
              <span className="text-muted-foreground">
                {product.latestPublishResult.modeLabel} on{" "}
                {product.latestPublishResult.createdAtLabel}
              </span>
            </div>
            ...
            <Card>
              <CardHeader className="flex flex-col gap-4 border-b bg-white/3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    formAction={saveReviewDraftAction}
                    type="submit"
                    variant="secondary"
                  >
                    Save Draft
                  ...
                  <Button
                    formAction={approveReviewedProductAction}
                    type="submit"
                  >
                    Approve
                  </Button>
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      min="0"
                      defaultValue={
                        product.compareAtPriceValue?.toFixed(2) ?? ""
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      defaultValue={
                        product.aiConfidence === null
                          ? ""
                          : Number(product.aiConfidence).toFixed(2)
                      }
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid gap-3">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      name="stylePresetId"
                      defaultValue={
                        stylePresetData.defaultPresetId ?? undefined
                      }
...
Do not call it a lighter`}
                  ...
                  <div className="grid gap-2">
                    <Button
                      formAction={regenerateProductImageAction}
                      type="submit"
                      variant="outline"
                      className="w-full"
                    >
                    ...
                    <Button
                      formAction={regenerateProductTextAction}
                      type="submit"
                      variant="outline"
                      className="w-full"
                    >
                    ...
                    <Button
                      formAction={regenerateProductPriceAction}
                      type="submit"
                      variant="outline"
                      className="w-full"
                    >
                    ...
                    <Button
                      formAction={tryDifferentStylePresetAction}
                      type="submit"
                      variant="outline"
                      className="w-full"
                    >
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="rounded-2xl border bg-black/20 p-4">
                ...
                <div className="mt-3 grid gap-2 text-xs text-muted-foreground">
                  <p>
                    Price:{" "}
                    {formatCurrency(Math.round(product.priceValue * 100))}
                  </p>
                  <p>
                  ...
                  <p>
                    Tags:{" "}
                    {product.tags.length ? joinList(product.tags) : "None"}
                  </p>
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
                Human approval remains required before publishing. AI changes
                only update the review draft.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground">
                Every regeneration attempt is saved with prompts, options, and
                restoreable outputs.
              </p>
              {product.aiGenerations.length ? (
                <div className="space-y-3">
                  {product.aiGenerations.map((generation) => (
                    <details
                      key={generation.id}
                      className="rounded-2xl border bg-black/20 p-4 text-sm"
                    >
                      <summary className="cursor-pointer list-none">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {generation.isSelectedFinal ? (
                              <Badge>Selected final</Badge>
                            ) : null}
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        {generation.options ? (
                          <div className="grid gap-1 text-xs text-muted-foreground">
                            ...
                            <p>
                              Price strategy: {generation.options.priceStrategy}
                            </p>
                          </div>
                        ) : null}
                        ...
                        {generation.reviewSnapshot?.metadata ? (
                          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-muted-foreground">
                            <p className="font-medium text-foreground">
                              {generation.reviewSnapshot.metadata.title}
                            </p>
                            <p className="mt-1">
                              {formatCurrency(
                                Math.round(
                                  generation.reviewSnapshot.metadata.price * 100
                                )
                              )}{" "}
