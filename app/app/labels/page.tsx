...
export default async function LabelsPage({
  ...
  const [products, labels] = await Promise.all([
    ...
    selectedIds.length
      ? getPrintableLabels(selectedIds, qrTarget)
      : Promise.resolve([])
  ]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-[var(--font-display)] text-4xl sm:text-5xl">
            Label generator
          </h1>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
            Build small printable product tags with SKU, price, category, and a
            QR code that resolves to studio inventory or the live product page.
          </p>
        </div>
        <div className="flex flex-wrap gap-2 print:hidden">
          ...
          >
            <Link href={buildLabelsHref("internal", selectedIds)}>
              Internal QR
            </Link>
          </Button>
          <Button
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between print:hidden">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Optimized for standard letter paper now, with reusable label
                components for future label-printer output.
              </p>
            </div>
          ...
          <LabelSheet
            labels={labels}
            emptyMessage="Select one or more products to build a label sheet."
          />
        </div>
      </div>
    </div>
