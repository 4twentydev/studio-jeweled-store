    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="grid min-w-[240px] gap-2 rounded-[1.75rem] border bg-white/4 p-4">
          ...
          <p className="font-[var(--font-display)] text-4xl">
            {formatCurrency(inventoryValueCents)}
          </p>
          <p className="text-sm text-muted-foreground">
            {product.quantityOnHand} units currently on hand
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_380px]">
            ...
            <CardHeader className="flex flex-col gap-4 border-b bg-white/3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Manage merchandising copy, pricing, stock, and notes for this
                  item.
                </p>
                {!canEdit ? (
                  <p className="mt-2 text-sm text-muted-foreground">
                    Editing is disabled while the app is running in demo mode.
                  </p>
                ) : null}
              </div>
              ...
              <div className="grid gap-5 lg:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    defaultValue={product.priceValue.toFixed(2)}
                  />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="materials"
                    name="materials"
                    defaultValue={joinList(product.materials)}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="colors"
                    name="colors"
                    defaultValue={joinList(product.colors)}
                    className="min-h-[110px]"
                  />
                </div>
                <div className="space-y-2 lg:col-span-2">
                  ...
                  <Textarea
                    id="tags"
                    name="tags"
                    defaultValue={joinList(product.tags)}
                    className="min-h-[110px]"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={
                      product.aiConfidence === null
                        ? ""
                        : product.aiConfidence.toFixed(2)
                    }
                  />
                </div>
              </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm">
              ...
              <div className="flex items-center justify-between rounded-[1.25rem] border bg-black/20 px-4 py-3">
                ...
                <span>
                  {product.hasMetadataGaps ? "Needs attention" : "Healthy"}
                </span>
              </div>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3">
              ...
              <Button asChild variant="outline">
                <Link href="/inventory/export?approvedOnly=true&format=json">
                  Export approved products
                </Link>
              </Button>
              <p className="text-sm text-muted-foreground">
                Only approved products can publish. Export mode prepares JSON or
                CSV for manual JWLD.store import.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3">
              <Button asChild>
                <Link href={`/app/labels/${product.id}`}>
                  Generate product label
                </Link>
              </Button>
              <Button asChild variant="outline">
                <Link href={`/app/labels?productIds=${product.id}`}>
                  Add to batch print sheet
                </Link>
              </Button>
              <p className="text-sm text-muted-foreground">
                Build a printable tag with title, SKU, price, category, and QR
                code for physical inventory labeling.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                      ...
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Badge variant="secondary">
                            {formatStatusLabel(image.imageKind)}
                          </Badge>
                        </div>
              ...
              <form
                action={replaceProcessedImageAction}
                className="grid gap-3 rounded-[1.5rem] border p-4"
              >
                <input type="hidden" name="productId" value={product.id} />
                <input type="hidden" name="redirectTo" value={redirectTo} />
                <div>
                  <p className="font-medium">
                    Upload replacement processed image
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Add a new polished image version and make it the primary
                    studio asset.
                  </p>
                </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {product.aiGenerations.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No AI history has been logged for this item yet.
                </p>
              ) : (
                product.aiGenerations.map((generation) => (
                  <div
                    key={generation.id}
                    className="rounded-[1.5rem] border bg-black/20 p-4"
                  >
          ...
          <Card>
            ...
            <CardContent>
              <div className="rounded-[1.5rem] border border-dashed bg-black/20 p-4 text-sm text-muted-foreground">
                Latest publish results now appear in the studio summary. A full
                event timeline can build on the saved `publish_results` records
                later without changing the publishing flow again.
              </div>
            </CardContent>
          </Card>
        </div>
